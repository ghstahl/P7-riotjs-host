...
riot.tag2('download-manager-home', '<div class="panel panel-default"> <div class="panel-heading">Download Manager</div> <div class="panel-body"> <div class="well"> This pulls download from /download-manager.json </div> <table if="{state.localData}" class="table table-striped table-hover "> <thead> <tr> <th>File</th> <th>Complete</th> <th>percentComplete</th> <th>link</th> </tr> </thead> <tbody> <tr each="{state.localData.items}"> <td>{this.fileName}</td> <td>{this.downloadItem.isComplete}</td> <td>{this.downloadItem.percentComplete}</td> <td if="{!this.downloadItem.isComplete && !this.downloadItem.isInProgress}"> <a href="{this.url}" download="{this.fileName}">Download</a></td> </td> <td if="{!this.downloadItem.isComplete && this.downloadItem.isInProgress}"> <a class="btn btn-default" onclick="{this.onCancel}">Cancel</a> </td> <td if="{this.downloadItem.isComplete}"> <a class="btn btn-default" onclick="{this.onInstall}">Install</a> </td> </tr> </tbody> </table> </div> </div>', '', '', function (opts) {
  var _this = this;
